...
public class Trump
{
	...
	public int setTrumpCall () {
        ...
        if ((cValueMap.keySet().toString().equals(cPointMap.keySet().toString())) && (cPointMap.keySet().toString().equals(cCountMap.keySet().toString()))
        	...
        	this.trumpSuit = cValueMap.keySet().toArray()[3].toString();
        	...
        	this.trumpSuit = cPointMap.keySet().toArray()[3].toString();
        	...
        	this.trumpSuit = cCountMap.keySet().toArray()[3].toString();
        } else {
        	this.trumpSuit = cValueMap.keySet().toArray()[3].toString();
        }
        
        // Add buffer to Trump Call Score
    	if (cPointMap.get(this.trumpSuit) < 3) {
    		bufferTrumpAddOn = 0;
    	} else if ((cPointMap.get(this.trumpSuit) > 3) && (cPointMap.get(this.trumpSuit) < 6)) {
    		bufferTrumpAddOn = 1;
    	} else if ((cPointMap.get(this.trumpSuit) > 6) && (cPointMap.get(this.trumpSuit) < 9)) {
    		bufferTrumpAddOn = 2;
    	} else if ((cPointMap.get(this.trumpSuit) > 9) && (cPointMap.get(this.trumpSuit) < 12)) {
    		bufferTrumpAddOn = 3;
    	}
    	System.out.println("");
    	
        // Find Lowest Card of Trump Suit to hide/fold
        if (this.trumpSuit.equals("Clubs")) {
        	trumpFaceValue = LowestClubs;
        } else if (this.trumpSuit.equals("Hearts")) {
        	trumpFaceValue = LowestHearts;
        } else if (this.trumpSuit.equals("Diamonds")) {
        	trumpFaceValue = LowestDiamonds;
        } else if (this.trumpSuit.equals("Spades")) {
        	trumpFaceValue = LowestSpades;
        }
        
        // Convert trumpFaceValue to Face
        switch (trumpFaceValue) {
        	case 7: this.trumpFace = "Seven" ;
        			break;
        	case 8: this.trumpFace = "Eight" ;
					break;
        	case 9: this.trumpFace = "Queen" ;
					break;
        	case 10: this.trumpFace = "King" ;
					break;
        	case 11: this.trumpFace = "Ten" ;
					break;
        	case 12: this.trumpFace = "Ace" ;
					break;
        	case 13: this.trumpFace = "Nine" ;
					break;
        	case 14: this.trumpFace = "Jack" ;
					break;
        }
	...
	public static int getTrumpCall() {
		return trumpCall;
	}

	public static void setTrumpCall(int trumpCall) {
		Trump.trumpCall = trumpCall;
	}
